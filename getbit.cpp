...
int main()
{
  ...
  unsigned char byte;     // return value for bitBuffer.loadbyte(byte)
  long nbit = 0;           // counter
  ...
  while (bitBuffer.loadbyte(byte)) {
    cout<< (unsigned int) byte << " ";
  }
  cout<<endl;
  bitBuffer.Print();

  // cout<< "\nExercise ungetbyte and loadbyte" <<endl;
  // int nrep = 10;
  // for (int ibyte=0; ibyte<nrep; ++ibyte) bitBuffer.ungetbyte();
  // for (int ibyte=0; ibyte<nrep; ++ibyte) {
  //   cout<< (unsigned) bitBuffer.loadbyte() <<endl;
  // }
  // return 0;
  ...
  //    cout<< "loadbyte again" <<endl;
  //    //-- for (int ibyte=0; ibyte<60; ++ibyte) {
  //    for (int ibyte=0; ibyte<256; ++ibyte) {
  //        if (bitBuffer.loadbyte(byte)) {
  //            cout<< (unsigned) (unsigned char) bitBuffer.loadbyte() << " ";
  //        }
  //        else {
  //            cout<< "*** no bytes left. Stop." <<endl;
  //            break;
  ...
  try {
    cout<< "Next call of the loadbyte will generate an exception" <<endl;
    bitBuffer.loadbyte();
  }
  ...
  for (int i=0; i<pattern_size; ++i) buf[i] = bitBuffer.loadbyte();
  ...
  while (!same) {
    ...
    buf[pattern_size-1] = bitBuffer.loadbyte();
    same = true
      && buf[0] == pattern[0]
      && buf[1] == pattern[1]
      && buf[2] == pattern[2]
  ...
  if (same) cout<< "The next element after the pattern is " << bitBuffer.loadbyte() <<endl;
