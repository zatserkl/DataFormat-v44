...
void readBinary(ifstream &infile)
{
  ...
  TTree* tree = new TTree("b", "bin tree");
  tree->Branch("t", &t, "t[4]/F");
  ...
  // for( int i = 0; i<num_histories && i<10; i++ )
  {
    ...
    if (i<10) printf("%8.4f \t %8.4f \t %8.4f \t %8.4f \t %8.4f \t %8.4f \t %8.4f \t %8.4f \t %8.4f \n",   //
...
int binaryTree(const char* fileName)
{
  ...
  TFile* ofile = new TFile(Form("%s.root",fileName), "recreate");
